...
import logging
from deploy.utils import load_tensorrt_plugin,search_cuda_version
...
class Model:
    ...
    def allocate_buffers(self):
        ...
        for binding in self.engine:  # input box conf
    ...
    def load_engine(self):
        ...
        def build_engine():
            ...
            with trt.Builder(self.TRT_LOGGER) as builder, \
                ...
                # builder.max_workspace_size = 1 << 33  # trt 7
                # config=builder.create_builder_config()  # trt 8
                ...
                if cuda_version is not None:
                    ...
                    if version_major < 11:
                        ...
                        tactic_source = builder_config.get_tactic_sources() - (
                ...
                if builder_config.add_optimization_profile(profile) < 0:
                    self.logger.warning(f'Invalid optimization profile {profile}.')
                ...
                if self.precision==32:
                    ...
                    if not getattr(builder, 'platform_has_fast_fp16', True):
                        self.logger.warning('Platform does not has fast native fp16.')
                    builder_config.set_flag(trt.BuilderFlag.FP16)
                elif self.precision==8:
                    if not getattr(builder, 'platform_has_fast_int8', True):
                        self.logger.warning('Platform does not has fast native int8.')
                ...
                if isinstance(self.onnx_file, str):
                    parse_valid = parser.parse_from_file(self.onnx_file)
                else:
    ...
    def pre_proces(self,images):
        imgs=[]
        for im in images:
            imgs.append(im)
        imgs = np.array(imgs)
        imgs = imgs.half() if self.half else im.float()  # uint8 to fp16/32
        imgs /=  self.alpha  # 0 - 255 to 0.0 - 1.0
        if len(imgs.shape) == 3:
    ...
    def pre_process(self, image_list):
        ...
        for src_img in image_list:
            if src_img is None:
                continue
            flt_img = None
            if self.INPUT_CHANNEL == 1:  # 模型适用 单通道图像
                flt_img = cv2.cvtColor(src_img, cv2.COLOR_RGB2GRAY)
            elif self.INPUT_CHANNEL == 3:  # 三通道图像
        ...
        image_batch = np.array(image_batch, dtype=np.float16, order='C')  #
        return image_batch
