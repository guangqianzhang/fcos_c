...
__global__ void warp_affine_bilinear_kernel(
    ...
    affine_project(matrix.d2i, dx, dy, &src_x, &src_y);  //线程idx投影到src_xy
    ...
    if(src_x < -1 || src_x >= src_width || src_y < -1 || src_y >= src_height){
        ...
        int y_low = floorf(src_y);//最近4个像素点
        int x_low = floorf(src_x);
        int y_high = y_low + 1;
        int x_high = x_low + 1;

        uint8_t const_values[] = {fill_value, fill_value, fill_value};//图片外的常量值
        ...
        if(y_low >= 0){
            if (x_low >= 0)
                v1 = src + y_low * src_line_size + x_low * 3;  //3=rgb
        ...
        c0 = (w1 * v1[0] + w2 * v2[0] + w3 * v3[0] + w4 * v4[0] + 0.5f);//保持float
...
//    pdst[0] = c0/255; pdst[1] = c1/255; pdst[2] = c2/255;//归一化
}
...
void warp_affine_bilinear(
    ...
    dim3 block_size(32, 32); // blocksize最大就是1024，这里用2d来看更好理解 n=32
    dim3 grid_size((dst_width + 31) / 32, (dst_height + 31) / 32); 
    AffineMatrix affine;
    affine.compute(Size(src_width, src_height), Size(dst_width, dst_height));  //计算得到变换矩阵d2i
